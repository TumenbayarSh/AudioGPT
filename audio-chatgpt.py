...
class T2I:
    def __init__(self, device):
        ...
        self.text_refine_model = AutoModelForCausalLM.from_pretrained("Gustavosta/MagicPrompt-Stable-Diffusion").to(device)
        # Use CPU for pipeline (device=-1)
        self.text_refine_gpt2_pipe = pipeline("text-generation", model=self.text_refine_model, tokenizer=self.text_refine_tokenizer, device=-1)
...
class Speech_Enh_SS_SC:
    def __init__(self, device="cpu", model_name="espnet/Wangyou_Zhang_chime4_enh_train_enh_conv_tasnet_raw"):
...
class Speech_SS:
    def __init__(self, device="cpu", model_name="lichenda/wsj0_2mix_skim_noncausal"):
        self.model_name = model_name
        self.device = device
...
class ConversationBot:
    ...
    def init_tools(self, interaction_type):
        # (Due to length, the tool setup remains the same as original code.)
        # Just ensure that tools are defined as in the original code snippet.
        # ... (Truncated for brevity, but unchanged logic)
        # Return the same gr.update() calls as before.
        ...
        if interaction_type == 'text':
            self.tools = [
                ...
                Tool(name="Generate Audio From User Input Text", func=self.t2a.inference,
                     description="..."),
                Tool(name="Style Transfer", func=self.tts_ood.inference,
                     description="..."),
    ...
    def run_text(self, text, state):
        # Implementation as per the original code snippet
        # (No GPU references here. Just logic remains same.)
        # ... (Truncated for brevity)
        pass

    def run_image_or_audio(self, file, state, txt):
        # Same logic as before, CPU-only inference (no GPU references).
        # ... (Truncated for brevity)
        pass

    def speech(self, speech_input, state):
        # Same logic as before, CPU-only.
        # ... (Truncated for brevity)
        pass
...
if __name__ == '__main__':
    ...
    with gr.Blocks(css="#chatbot .overflow-y-auto{height:500px}") as demo:
        # Implement the gradio interface as in the original code
        # ensuring no GPU assumptions are made.
